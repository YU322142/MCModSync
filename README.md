...
在 NeoForge 1.21.1 启动阶段，MCSync 会复用 NeoForge 的早期加载窗口显示检查、下载、缓存和哈希校验进度；不会额外弹出同步窗口。为适配 NeoForge 早期窗口的字体限制，这一阶段使用可显示的英文文件/状态标签，完整的中英文信息仍写入日志和 `.modsync` 状态文件。校验完成后 Minecraft 会退出，隐藏 helper 只负责对已校验内容执行原子提交，因此退出后的提交阶段不再拥有 Minecraft 窗口。无法提供早期加载窗口的 Fabric、移动端或辅助进程环境会回退到标题、日志和 `.modsync` 状态文件。Minecraft 窗口内的推荐内容选择仍在可用的游戏界面阶段显示。
