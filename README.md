...
当前公开发布版本为 `1.9.0`，要求 Java 21、Fabric Loader `>=0.15.11`，并精确支持 Minecraft `1.21.1` 与 `1.21.11`。同一个公开 JAR 可在这两个目标版本运行；源码同时保留 v1/v2/v3 读取兼容性。Fabric Loader 和 Minecraft 版本匹配仍由实例自身负责，不能把一个版本的 Mod 清单装入另一个版本的实例。

电脑端便携更新完成后，置顶进度窗口会明确显示双语 `Restart Required` 提示，并等待玩家点击“确定，返回启动器”；不再于 5 秒后自动消失。手机端及无图形环境继续只写入日志和状态文件，不显示 Swing 弹窗。

推荐把同一个 Minecraft 目标版本的旧电脑地址、旧手机地址和新版目录分开部署；下面以 1.21.1 为例，1.21.11 必须复制同样结构到独立目录：
...
每份清单中的文件名都按该清单 URL 的目录解析。旧电脑和旧手机目录各自只需发布 `mods.txt`、当前同步器和配置引导 JAR；目标版本目录发布该版本的 `mods-v4.txt` 与其列出的全部 JAR。两个旧地址可以相同，也可以像旧版本那样分开；它们最终都由配置引导 JAR 切到同一个目标版本地址。`mods.txt` 是永久升级入口，不要改成 v4 或删除。1.21.11 需要另建 `/minecraft/1.21.11/`，不能复用这里的 1.21.1 清单。

### 1.21.1 与 1.21.11 并行部署

1.9.0 的代码不调用 Minecraft 游戏类，只通过 Fabric Loader 的 PreLaunch 接口和 Java 21 标准库工作，因此同一公开 JAR 可以同时服务两个目标版本。但“同步器相同”不等于“模组清单相同”：两个版本必须各自维护独立的 URL、目录、`modsync.properties`、`mods.txt`/`mods-v4.txt`、`catalog-version` 历史和 Mod JAR 集合。推荐的服务器布局如下：
...
发布目录的父目录被视为游戏根目录。例如 `D:\Release\1.21.1-fabric\mods` 的配置模板必须是 `D:\Release\1.21.1-fabric\modsync.properties`（1.21.11 使用同样布局）。把当前 `MCModSync-1.9.0.jar` 放进 `mods` 目录；发布器会把它作为必须模组写入完整 v4，并与固定名 `MCModSync-Config.jar` 一起写入升级专用 v2。
...
运行 `MCModSync-1.9.0.jar`，选择已经测试完成的 `mods` 目录，然后打开必须/推荐清单编辑器。
...
```powershell
java -jar MCModSync-1.9.0.jar "D:\Release\1.21.1-fabric\mods"
```

默认输出为 `mods-v4.txt`，并在 `mods` 目录旁生成永久 `mods.txt`。也可以指定 v4 输出路径：

```powershell
java -jar MCModSync-1.9.0.jar "D:\Release\1.21.1-fabric\mods" "D:\Publish\mods-v4.txt"
```
...
https://files.example.com/minecraft/1.21.1/
├─ mods-v4.txt
├─ MCModSync-1.9.0.jar
├─ MCModSync-Config.jar
...
3. 下载并校验 `MCModSync-1.9.0.jar`，按 Fabric Mod ID `mcmodsync` 自动替换旧版本，同时下载固定名 `MCModSync-Config.jar`。
4. 本次以退出码 `0` 正常结束，启动器显示正常退出。
5. 再启动时，1.9.0 从配置引导 JAR 自动创建/更新游戏根目录的 `modsync.properties`，保留本地语言、超时、重试、文件大小限制等本地键，并读取所属 Minecraft 版本的 `mods-v4.txt`。
...
```powershell
java -jar MCModSync-1.9.0.jar --upgrade-v2 "D:\Release\1.21.1-fabric\mods"
```

这个命令默认写入 `D:\Release\1.21.1-fabric\mods\mods.txt`，仍会读取配置模板并确保配置引导 JAR 在清单中。把生成的 `mods.txt`、当前同步器和配置引导 JAR 复制到 1.21.1 对应的旧 URL 目录；1.21.11 工作区另行执行并上传到 `/minecraft/1.21.11/`。
...
# client-config.manifest=https://files.example.com/minecraft/1.21.1/mods-v4.txt
...
1. 把 `MCModSync-1.9.0.jar` 放入实例 `mods` 目录。
...
1. 从实例 `mods` 删除 `MCModSync-1.9.0.jar` 和 `MCModSync-Config.jar`。
...
```powershell
java -jar MCModSync-1.9.0.jar --resourcepack "D:\Publish\server-pack.zip"
java -jar MCModSync-1.9.0.jar --serverlist "D:\Publish\servers.dat"
```
...
如果启动器控制台出现 `Could not find or load main class io.github.mcmodsync.PortableUpdateHelper`，先升级到当前 1.9.0（路径修复自 1.8.6 起包含）。真实故障包已证明 1.8.5 的辅助 JAR 完整、SHA256 正确且主类存在；故障位于该 Windows Java 环境解析/装载完整 `-cp` 启动路径的阶段，而不是 JAR 下载或内容，带分号的完整路径也已在回归测试中复现同类错误。1.8.6 起改用隔离的 `helper-runtime-v2`、`.part` 原子提交、显式新时间戳和 MD5/SHA256 双校验；子 Java 以辅助目录为工作目录，只接收 ASCII JAR 文件名并通过 `-jar` 启动。父进程必须收到子进程的 ready 信号后才正常退出；提前退出、PID 异常或 10 秒超时会终止子进程、阻止本次启动并把明确原因写入日志。升级无需删除配置、清单或推荐选择记录。
...
脚本会编译主类和测试、运行完整测试、构建 `build/dist/MCModSync-1.9.0.jar`，并执行正常退出、便携模式、资源同步和清单兼容性检查。要验证真实旧版升级：
