...
在 NeoForge 1.21.1 启动阶段，MCSync 会复用 NeoForge 的早期加载窗口显示阶段、当前文件、百分比、完成状态和错误信息；不会额外弹出同步窗口。无法提供早期加载窗口的 Fabric、移动端或辅助进程环境会回退到标题、日志和 `.modsync` 状态文件。Minecraft 窗口内的推荐内容选择仍在可用的游戏界面阶段显示。
...
- CurseForge 查询使用当前 JAR 的官方 **fingerprint**。
...
可在“发布项目”页选择上一版 `mods-v5.json`。导出时会按大小与 SHA-256 比对：内容未变化的本地托管文件直接复用上一版不可变 URL，不再次复制进新版本目录；只有新增或变化文件需要上传。输出根目录同时生成 `UPLOAD-PLAN.json`、`UPLOAD-GUIDE.zh-CN.md` 和内容完全等价的 `UPLOAD-GUIDE.en.md`，明确上传、复用、外部下载和已移除路径。
