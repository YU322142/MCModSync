...
发布工作台的“远端与旧版升级”页提供两种方式：完整发布时勾选“生成永久升级入口”，会把 `legacy/` 一起写入发布目录；已经完成 v5 发布时，可指定一个新的空目录并点击“仅生成旧版入口上传包”，它不会重新扫描客户端或重建对象库。独立包会生成 `legacy-artifacts.json`、`SHA256SUMS.txt` 以及中英文上传说明。上传后永久入口为：
