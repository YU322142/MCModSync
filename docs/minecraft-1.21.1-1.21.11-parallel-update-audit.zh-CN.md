# MCModSync 1.9.1：Minecraft 1.21.1 / 1.21.11 并行更新可行性审计

审计日期：2026-08-10

## 一、结论

公开版 `MCModSync-1.9.1.jar` 可以作为同一个同步器二进制文件用于 Fabric Minecraft `1.21.1`/`1.21.11`，以及 NeoForge Minecraft `1.21.1`。运行要求为 Java 21；Fabric 元数据声明 Loader `>=0.15.11`，NeoForge 元数据声明 `javafml`、Minecraft `[1.21.1]` 和 NeoForge `[21.1.0,)`。公开 JAR 不使用 `>=1.21.1` 之类会意外放行其他版本的范围。
...
这里的“同一个公开 JAR”只指三个目录中的 `MCModSync-1.9.1.jar` 字节可以相同。`MCModSync-Config.jar` 应分别从各自游戏根目录的模板生成，因为它写入的 `manifest` URL 不同。其他 JAR 即使文件名相同，也必须按目标版本和加载器分别验证并计算 MD5/SHA256。
...
4. 检查清单中的同步器版本为 1.9.1，检查所有 JAR 的 MD5/SHA256，检查 `client-config.manifest` 指向本版本 URL。
...
旧地址继续永久提供只包含同步器和配置引导 JAR 的 `mods.txt`。升级后，1.9.1 从固定名 `MCModSync-Config.jar` 创建或更新游戏根目录的 `modsync.properties`，以后只读取该目标版本/加载器的新地址。手机端与电脑端可在同一组合内合并 URL，但不能把不同 Minecraft 版本或加载器合并。
...
- 不放入公开 1.9.1 的 `mods-v4.txt`，也不放入任何 1.21.1 目录；
- 保存私有 JAR 的 SHA256、部署日期、目标旧地址和撤回方案。

该阻止版不是普通并行更新节点。一旦某个 1.21.11 地址切换到阻止版，该地址的业务目标就是引导迁移，而不是继续同步 1.21.11 Mod。仍要运营 1.21.11 的其他实例必须使用另一个独立 URL 和普通公开版 1.9.1。

## 十、发布验收表

1. 在 JAR 内检查 Fabric `fabric.mod.json` 和 NeoForge `META-INF/neoforge.mods.toml`：版本 1.9.1；Fabric Minecraft 精确为 1.21.1/1.21.11；NeoForge Minecraft 精确为 1.21.1；Java >=21。
